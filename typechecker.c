...
static inline void reuse_push(Type** cache, bool is_vec_push, Type type) {
  ...
  if (is_vec_push && cache_header->size == cache_header->capacity) {
...
static void type_of_types(ASTNode** expr, signed short* ctx, Type** cache) {
...
static bool type_eq(Type a_type, Type b_type) {
  bool is_a_var;
  if (a_type < 0 && (is_a_var = true) && b_type < 0 || a_type > 0 && b_type > 0)
...
bool checker(TrieNode* astrie, TrieNode* _type_trie, TrieNode* _instance_trie, Error* error_buf) {
  _Static_assert(sizeof(TypeVar) == sizeof(Type), "Type and TypeVar do not have the same size");
